...
void Bug::Update() {
  ...
  // Update all of the body vector items if the bug head has moved to a new
  // cell.
...
void Bug::UpdateHead() {
  ...
  // Wrap the Bug around to the beginning if going off of the screen.
  head_x = fmod(head_x + grid_width, grid_width);
  head_y = fmod(head_y + grid_height, grid_height);
}

void Bug::UpdateBody(SDL_Point &current_head_cell,
...
void Bug::GrowBody() { growing = true; }

// Inefficient method to check if cell is occupied by bug.
bool Bug::BugCell(int x, int y) {
...
void Bug::FindMovesFromPath(std::vector<Route::Node> path) {
  this->moves.clear();
